...
namespace Test {
  ...
  test_points()
  {
    ...
    std::transform(boxstore.begin(),
                   boxstore.end(),
                   std::back_inserter(prim_boxes),
    ...
    using searcher
        = std::function<const ObjectBBox*(const ObjectBBox::vertex_type&,
    ...
    auto bench = [](const std::vector<ObjectBBox::vertex_type>& points,
      ...
      for (const auto& v : points) {
        hits.push_back(search(v, prims, n_intersects, world));
      }
    ...
    csv_centers
        << "n_boxes,octree_depth,n_int_dumb,n_int_bvh,diff_dumb,diff_bvh"
        << std::endl;
    csv_centers << std::setprecision(6);

    std::ofstream csv_rand("random.csv");
    csv_rand << "n_points,n_boxes,octree_depth,n_int_dumb,n_int_bvh,diff_dumb,"
                "diff_bvh"
             << std::endl;
    ...
    for (size_t n_boxes_1d = 4; n_boxes_1d < n_boxes_1d_max; n_boxes_1d++) {
      ...
      for (size_t octree_depth : octree_depths) {
        ...
        std::cout << "n_box_1D: " << n_boxes_1d << " oct_d: " << octree_depth
                  << std::endl;
        ...
        std::transform(lboxstore.begin(),
                       lboxstore.end(),
                       std::back_inserter(lprim_boxes),
                       [](const auto& box) { return box.get(); });
        ...
        std::tie(n_int_dumb, diff_dumb, hits_dumb)
            = bench(centers, lprim_boxes, ltop, dumb_search);
        std::tie(n_int_bvh, diff_bvh, hits_bvh)
            = bench(centers, lprim_boxes, ltop, bvh_search);
        compare_hits(centers, hits_dumb, hits_bvh);

        csv_centers << n_boxes_1d << "," << octree_depth << "," << n_int_dumb
                    << "," << n_int_bvh << "," << diff_dumb << "," << diff_bvh
                    << std::endl;
        ;

        std::cout << "o centers: dumb/bvh: " << diff_dumb << "s " << diff_bvh
                  << "s, " << n_int_dumb << ", " << n_int_bvh;
        std::cout << std::endl;

        std::uniform_real_distribution<ObjectBBox::value_type> dist(
            ltop->min().minCoeff() * 0.9, ltop->max().maxCoeff() * 1 / 0.9);
        ...
        for (size_t size : npoints) {
          ...
          std::tie(n_int_dumb, diff_dumb, hits_dumb)
              = bench(rand_points, lprim_boxes, ltop, dumb_search);
          std::tie(n_int_bvh, diff_bvh, hits_bvh)
              = bench(rand_points, lprim_boxes, ltop, bvh_search);
          compare_hits(rand_points, hits_dumb, hits_bvh);

          csv_rand << size << "," << n_boxes_1d << "," << octree_depth << ","
                   << n_int_dumb << "," << n_int_bvh << "," << diff_dumb << ","
                   << diff_bvh << std::endl;
          ;

          std::cout << "o rand: size: " << size << " dumb/bvh: " << diff_dumb
                    << "s " << diff_bvh << "s, " << n_int_dumb << ", "
                    << n_int_bvh;
          std::cout << std::endl;
        }
      }
  ...
  test_rays()
  {
    ...
    using searcher = std::function<std::vector<const ObjectBBox*>(
        const Ray&,
        const std::vector<ObjectBBox*>&,
        size_t&,
        const ObjectBBox*)>;
    ...
    auto bench = [](const std::vector<Ray>&         rays,
                    ...
                    const ObjectBBox*               world,
                    searcher                        search) -> std::
        tuple<size_t, double, std::vector<std::vector<const ObjectBBox*>>> {
    ...
    auto compare_hits = [](
      ...
      for (size_t i = 0; i < a.size(); i++) {
        ...
        std::set_difference(hits_set_a.begin(),
                            hits_set_a.end(),
                            hits_set_b.begin(),
                            hits_set_b.end(),
    ...
    csv_rand
        << "n_rays,n_boxes,octree_depth,n_int_dumb,n_int_bvh,diff_dumb,diff_bvh"
        << std::endl;
    csv_rand << std::setprecision(6);

    for (size_t n_boxes_1d = n_boxes_1d_min; n_boxes_1d < n_boxes_1d_max;
         n_boxes_1d++) {
      if (n_boxes_1d % 2 != 0) {
        continue;
      }
      ...
      for (size_t octree_depth : octree_depths) {
        ...
        // std::tie(n_int_dumb, diff_dumb, hits_dumb) = bench(rays, lprim_boxes,
        // ltop, dumb_search);
        std::tie(n_int_bvh, diff_bvh, hits_bvh)
            = bench(rays, lprim_boxes, ltop, bvh_search);
        // compare_hits(rays, hits_dumb, hits_bvh);

        csv_rand << nrays << "," << n_boxes_1d << "," << octree_depth << ","
                 << n_int_dumb << "," << n_int_bvh << "," << diff_dumb << ","
                 << diff_bvh << std::endl;
        ;
        std::cout << " rand: nrays: " << nrays << " dumb/bvh: " << diff_dumb
                  << "s " << diff_bvh << "s, " << n_int_dumb << ", "
                  << n_int_bvh;
