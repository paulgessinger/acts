...
class MeasurementSelector {
 public:
  ...
  select(std::vector<MultiTrajectory::TrackStateProxy>& candidates,
    ...
    // Need covariance to compute chi2
    // if (not candidates.front().predictedCovariance().()) {
    // //   return CombinatorialKalmanFilterError::MeasurementSelectionFailed;
    // // }
    ...
    //         const auto& H = meas.projector();
    //         // Take the parameter covariance
    //         const auto& predictedCovariance =
    //         *predictedParams.covariance();
    //         // Get the residuals
    //         const auto& res =
    //         meas.residuals(predictedParams.parameters());
    //         // Get the chi2
    //         double chi2 =
    //             (res.transpose() *
    //              ((meas.covariance() + H * predictedCovariance *
    //              H.transpose()))
    //                  .inverse() *
    //              res)
    //                 .eval()(0, 0);
    ...
    //         ACTS_VERBOSE("Chi2: " << chi2);
    //         // Push the measurement index and chi2 if satisfying the
    //         criteria if (chi2 < chi2CutOff) {
    //           measChi2.at(nInitialCandidates) = {index, chi2};
    //           nInitialCandidates++;
    //         }
    //         // Search for the measurement with the min chi2
