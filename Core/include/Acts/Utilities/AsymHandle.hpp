...
class AsymHandle {
 ...
 public:
  ...
  AsymHandle& operator=(const AsymHandle& other) {
    m_ptr = other.m_ptr;
    m_cb = other.m_cb;
    increment();
    return *this;
  };
  AsymHandle& operator=(AsymHandle&& other) {
    m_ptr = other.m_ptr;
    m_cb = other.m_cb;
    other.reset();
    // don't need to increment, ref count stays the same
  ...
  template <typename T,
            typename = std::enable_if_t<std::is_base_of_v<element_t, T>>>
  AsymHandle(const AsymHandle<T>& other) {
    m_ptr = other.m_ptr;
    m_cb = other.m_cb;
    increment();
  }
 ...
 private:
  void increment() {
    std::cout << "incr: " << m_cb->refCount << " -> " << (m_cb->refCount + 1)
              << std::endl;
    m_cb->refCount++;
  }
  void decrement() {
    if (m_cb == nullptr || m_ptr == nullptr) {
      return;
    }
    std::cout << "decr: " << m_cb->refCount << " -> " << (m_cb->refCount - 1)
              << std::endl;
    if (m_cb->refCount.fetch_sub(1) == 1) {
 ...
 public:
  element_t& operator*() { return *m_ptr; }
  element_t* operator->() { return m_ptr; }
  operator bool() const { return m_ptr != nullptr; }
  element_t* get() { return m_ptr; }

  const element_t& operator*() const { return *m_ptr; }
  const element_t* operator->() const { return m_ptr; }
  const element_t* get() const { return m_ptr; }

  size_t refCount() const { return m_cb->refCount; }
...
AsymHandle<T> makeAsymHandle(Args&&... args) {
  ...
  return AsymHandle<T>{std::move(up)};
}
