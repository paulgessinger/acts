...
public:
  ...
  draw(helper_t& helper, value_type far_distance = 10) const
  {
    ...
    static_assert(
        std::is_same<typename helper_t::value_type, value_type>::value,
    ...
    // l2g = l2g *
    // Eigen::Quaternion<value_type>().setFromTwoVectors(vertex_type::UnitZ(),
    // normal);
    ...
    vertex_type far_normal
        = m_normals[0];  // far has same normal as pseudo-near
    ...
    for (size_t i = 1; i < m_normals.size(); i++) {
      ...
      // std::cout << ixLine.first.transpose() << " " <<
      // ixLine.second.transpose() << std::endl;
      // std::cout << "idx: " << i << std::endl;
      planeFarIXs.at(i - 1) = ixLine;

      // draw_line(m_origin+ixLine.second-ixLine.first*15,
      // m_origin+ixLine.second+ixLine.first*15);
    }
    ...
    for (size_t i = 0; i < planeFarIXs.size(); i++) {
      ...
      // planeFarIXs.at(i).intersection(planeFarIXs.at(j));
      ...
      value_type     unit         = 20.) const
  {
    ...
    // draw_line(m_origin+far_point - 5*far_dir, m_origin+far_point + 5*far_dir,
    // "black", 2);
...
std::ostream&
operator<<(std::ostream& os, const Ray<U, V>& ray)
{
  ray.dump(os);
  return os;
}
...
public:
  ...
  template <size_t D = DIM, std::enable_if_t<D == 3, int> = 0>
  void
  obj(std::ostream& os, size_t& vtx_offset) const
  {
            ...
            typename box_t::value_type           envelope1 = 0)
{
  ...
  oct = [&store, &max_depth, &oct, &envelope](const std::vector<box_t*>& lprims,
                                              size_t depth) -> box_t* {
