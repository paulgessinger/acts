  ...
  if (numcLayers) {
    ...
    for (size_t icl = 0; icl < numcLayers; ++icl) {
      ...
      double layerEnvelopeCoverZ =
          m_cfg.centralLayerEnvelopes.size()
      ...
      std::shared_ptr<const Acts::PlanarBounds> moduleBounds =
          std::make_shared<const Acts::RectangleBounds>(moduleHalfX,
                                                        moduleHalfY);
  ...
  if (numpnLayers) {
    ...
    for (size_t ipnl = 0; ipnl < numpnLayers; ++ipnl) {
      ...
      for (auto& discModulePositions : m_cfg.posnegModulePositions.at(ipnl)) {
        ...
        // (1) module bounds
        // create the bounds
        std::shared_ptr<const Acts::PlanarBounds> moduleBounds = nullptr;
        if (moduleMaxHalfX != 0. && moduleMinHalfX != moduleMaxHalfX) {
          moduleBounds = std::make_shared<const Acts::TrapezoidBounds>(
              moduleMinHalfX, moduleMaxHalfX, moduleHalfY);
        } else {
          moduleBounds = std::make_shared<const Acts::RectangleBounds>(
              moduleMinHalfX, moduleHalfY);
        }
